...
import { useGLTF, Environment, Lightformer, Float, Center, ContactShadows } from '@react-three/drei'
 ...
 * Un objeto grande rotando/flotando + selector para cambiar entre los 4.
...
function Spinner({ file, scale, reduced }: { file: string; scale: number; reduced: boolean }) {
  const ref = useRef<THREE.Group>(null!)
  useFrame((_, d) => {
    if (ref.current && !reduced) ref.current.rotation.y += Math.min(d, 0.05) * 0.42
  })
  return (
  ...
  return (
    ...
    >
      <Suspense fallback={null}>
        ...
        <Spinner file={active.file} scale={active.scale} reduced={reduced} />

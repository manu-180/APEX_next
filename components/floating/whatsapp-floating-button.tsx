'use client'

import { useCallback, useEffect, useState } from 'react'
import { useRouter, usePathname } from 'next/navigation'
import { whatsappUrl, WA_MSG_NAV } from '@/lib/whatsapp'
import { openWhatsAppWithThankYouPage } from '@/lib/whatsapp-navigate'
import { trackGoogleAdsWhatsAppClick } from '@/lib/analytics/google-ads'
import { trackMetaLead } from '@/components/analytics/meta-pixel'
import { ROUTES } from '@/lib/constants'
import { cn } from '@/lib/utils/cn'

/**
 * Botón flotante de WhatsApp — visible mobile + tablet, oculto en /contacto
 * (porque ahí la página ya muestra el canal). Mobile-first: bottom-right
 * con safe-area-inset, hit-area >= 56px, ripple pulse para call-to-action,
 * y label expandible en tablet+.
 *
 * Trackea conversión Google Ads + Meta Lead al click.
 */
export function WhatsAppFloatingButton() {
  const router = useRouter()
  const pathname = usePathname()
  const [mounted, setMounted] = useState(false)
  const [scrolled, setScrolled] = useState(false)

  // Render diferido para no bloquear LCP — aparece tras hidratación
  useEffect(() => {
    setMounted(true)
  }, [])

  // Detectar scroll para acortar label ("Hablemos" → solo icono al scrollear mucho)
  useEffect(() => {
    if (typeof window === 'undefined') return
    let raf = 0
    const handler = () => {
      cancelAnimationFrame(raf)
      raf = requestAnimationFrame(() => setScrolled(window.scrollY > 320))
    }
    window.addEventListener('scroll', handler, { passive: true })
    return () => {
      window.removeEventListener('scroll', handler)
      cancelAnimationFrame(raf)
    }
  }, [])

  const handleClick = useCallback(
    (e: React.MouseEvent<HTMLButtonElement>) => {
      e.preventDefault()
      trackGoogleAdsWhatsAppClick()
      trackMetaLead()
      openWhatsAppWithThankYouPage(whatsappUrl(WA_MSG_NAV), router)
    },
    [router],
  )

  // No mostrar en /contacto ni en /gracias (canal ya disponible o ya convirtió)
  const hide = pathname === ROUTES.contact || pathname === ROUTES.gracias

  if (!mounted || hide) return null

  return (
    <div
      className={cn(
        'fixed z-[60] pointer-events-none',
        'right-[max(1rem,env(safe-area-inset-right))]',
        'bottom-[max(1rem,calc(env(safe-area-inset-bottom)+0.5rem))]',
      )}
      aria-hidden={false}
    >
      <button
        type="button"
        onClick={handleClick}
        aria-label="Abrir WhatsApp con Manuel"
        className={cn(
          'pointer-events-auto group relative inline-flex items-center gap-2.5',
          'rounded-full px-4 py-3 sm:px-5 sm:py-3.5',
          'min-h-[3.5rem] min-w-[3.5rem]',
          'shadow-[0_8px_24px_-4px_rgba(0,0,0,0.4),0_0_0_1px_rgba(255,255,255,0.06)_inset]',
          'transition-all duration-200 ease-out active:scale-95',
          'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-black',
        )}
        style={{
          background: 'linear-gradient(135deg, #25D366 0%, #128C7E 100%)',
          color: 'white',
        }}
      >
        {/* Ripple pulse — anillo expansivo CTA */}
        <span
          aria-hidden
          className="absolute inset-0 rounded-full opacity-70 motion-safe:animate-[wa-ping_2.4s_cubic-bezier(0,0,0.2,1)_infinite]"
          style={{
            background: 'radial-gradient(circle, rgba(37, 211, 102, 0.55) 0%, transparent 65%)',
          }}
        />

        {/* Icono WhatsApp oficial */}
        <svg
          viewBox="0 0 32 32"
          className="relative size-7 shrink-0 drop-shadow-[0_1px_1px_rgba(0,0,0,0.25)]"
          fill="currentColor"
          aria-hidden
        >
          <path d="M19.11 17.205c-.372 0-1.088 1.39-1.518 1.39a.63.63 0 0 1-.315-.1c-.802-.402-1.504-.817-2.163-1.447-.545-.516-1.146-1.29-1.46-1.963a.426.426 0 0 1-.073-.215c0-.33.99-.945.99-1.49 0-.143-.73-2.09-.832-2.335-.143-.372-.214-.487-.6-.487-.187 0-.36-.043-.53-.043-.302 0-.53.115-.746.315-.688.645-1.032 1.318-1.06 2.264v.114c-.015.99.472 1.977.917 2.78.93 1.6 2.323 3.156 3.97 4.156.747.456 2.5 1.39 3.504 1.39 1.21 0 1.99-.93 1.99-1.748 0-.157-.043-.286-.073-.443-.115-.5-1.04-.7-1.7-.9z" />
          <path d="M16 0C7.156 0 0 7.155 0 16c0 2.985.83 5.79 2.265 8.193L.024 31.999l8.193-2.265A15.93 15.93 0 0 0 16 32c8.844 0 16-7.156 16-16S24.844 0 16 0zm0 29.234a13.36 13.36 0 0 1-6.93-1.93l-.5-.297-5.07 1.404 1.404-5.07-.298-.5A13.193 13.193 0 0 1 2.766 16C2.766 8.69 8.69 2.766 16 2.766S29.234 8.69 29.234 16 23.31 29.234 16 29.234z" />
        </svg>

        {/* Label expandible — visible siempre arriba del fold; condensa al scrollear o en mobile chico */}
        <span
          className={cn(
            'relative whitespace-nowrap font-semibold text-sm tracking-wide',
            'overflow-hidden transition-[max-width,opacity,margin] duration-300 ease-out',
            'hidden xs:inline-block sm:inline-block',
            scrolled ? 'max-w-0 opacity-0 -ml-2.5' : 'max-w-[10rem] opacity-100',
          )}
        >
          Hablemos
        </span>
      </button>

      <style jsx>{`
        @keyframes wa-ping {
          0% {
            transform: scale(1);
            opacity: 0.7;
          }
          75%,
          100% {
            transform: scale(1.55);
            opacity: 0;
          }
        }
      `}</style>
    </div>
  )
}
